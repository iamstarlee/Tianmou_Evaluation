...
def main():
    ...
    # 设置输入文件夹、基准图像、输出文件夹
    input_folder = "multi/300/denoised_output_cam0/td"  # npy 文件夹
    ...
    img2 = cv2.flip(cv2.resize(img2, (320, 320)), 1)   # 统一到 160x160
    if img2 is None:
    ...
    for file_name in input_folder[start_idx::step]:
        ...
        # 加载当前图像 (npy)
        img1 = cv2.resize(np.load(input_path), (640, 320))
        if img1 is None:
        ...
        # 应用仿射变换
        warped_img1 = cv2.warpAffine(img1_disp, M, (320, 320))

        # 拼接 warped 和 GT
        concat_img = np.hstack((img2, warped_img1))
        cv2.imshow("res", concat_img)
        cv2.waitKey(0)

        # 保存结果为 JPG
        rel_path = os.path.splitext(file_name)[0] + "_compare.jpg"
