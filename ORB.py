...
# ---------------- ORB 检测 ----------------
def detect_orb_points(img, nfeatures=200, roi_ratio=0.45, min_distance=20):
    """
    ORB 检测关键点
    :param img: 灰度图 uint8
    :param nfeatures: 最大特征点数
    :param roi_ratio: ROI 半径比例
    :param min_distance: 关键点最小间距
    :return: keypoints
    """
    h, w = img.shape
    ...
    keypoints = orb.detect(img, mask)

    # 距离过滤
    keypoints = filter_keypoints_distance(keypoints, min_distance=min_distance)
    return keypoints

# ---------------- 主程序 ----------------
folder_l = 'multi/150/cone_output_cam0'
name = "ORB"
...
for idx, file_l in enumerate(files_l[::step]):  
    ...
    # ORB 检测关键点
    keypoints = detect_orb_points(gray, nfeatures=300, roi_ratio=0.425, min_distance=8)
    corner_counts.append(len(keypoints))

    # 绘制关键点
    out_img = cv2.drawKeypoints(vis, keypoints, None, color=(64,64,255))
